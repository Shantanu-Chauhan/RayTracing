...
bool Cylinder::Intersect(Ray* ray, Intersection& intersection)
{
	...
	t0 = std::max(std::max(a0.T0,b0.T0),c0.T0);
	t1 = std::min(std::min(a0.T1,b0.T1),c0.T1);
	if (t0 > t1)
		return false;
	else
	{
...
Triangle::Triangle(MeshData* meshdata)
{
	for (int i = 0; i < meshdata->triangles.size(); i++)
	{
		...
		//auto test = meshdata->triangles[i];
		one = meshdata->triangles[i].x();
		two = meshdata->triangles[i].y();
		three = meshdata->triangles[i].z();
		v0.push_back(meshdata->vertices[one].pnt);
		v1.push_back(meshdata->vertices[two].pnt);
		v2.push_back(meshdata->vertices[three].pnt);
